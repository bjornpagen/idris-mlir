...
The `CoreSyn` type, and the functions that operate over it, gets an entire directory [compiler/GHC/Core](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core):

- [compiler/GHC/Core.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core.hs): the data type itself.

- [compiler/GHC/Core/Ppr.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Ppr.hs): pretty-printing.
- [compiler/GHC/Core/FVs.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/FVs.hs): finding free variables.
- [compiler/GHC/Core/Subst.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Subst.hs): substitution.
- [compiler/GHC/Core/Utils.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Utils.hs): a variety of other useful functions over Core.

- [compiler/GHC/Core/Unfold.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Unfold.hs): dealing with "unfoldings".

- [compiler/GHC/Core/Lint.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Lint.hs): type-check the Core program. This is an incredibly-valuable consistency check, enabled by the flag `-dcore-lint`.

- [compiler/GHC/Core/Tidy.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Tidy.hs): part of the [the CoreTidy pass](commentary/compiler/hsc-main) (the rest is in [compiler/GHC/Iface/Tidy.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Iface/Tidy.hs)).
- [compiler/GHC/CoreToStg/Prep.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/CoreToStg/Prep.hs): [the CorePrep pass](commentary/compiler/hsc-main)
...
- The `res_ty` type is the same as the type of any of the right-hand sides (up to refining unification -- coreRefineTys in [compiler/GHC/Core/Unify.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Unify.hs) -- in pre-[FC](commentary/compiler/fc)).
...
[compiler/GHC/Core/Subst.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Subst.hs)), which maintains the list of
...
If you are interested in the way Core is translated into human readable form, you should check the sources for [compiler/GHC/Core/Ppr.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/GHC/Core/Ppr.hs). It is especially useful if you want to see how the Core data types are being built, especially when there is no Show instance defined for them.
