...
- `!idr.erased` maps to no values (1:0);
...
- **LOW-DATA-1 (v0).** `layout(T)` for `idr.data @T` with constructors
  ...
  1. **Components of a field.** An integer field is one component. An
     `!idr.erased` field has none. An `!idr.data<@U>` field contributes the
...
- **LOW-CRASH-1 (v0).** A crash lowers to three `llvm` ops:
...
- **LOW-EXT-1 (v0).** The only external symbols the object file may
  reference are `write` and `_exit`. The toolchain's `crt` files provide the
  process entry.
...
- **LOW-ENTRY-1 (v0).** `idr-lower` generates a public
  `func.func @main() -> i32`. It calls the `idr.entry` function, truncates the
  `i64` result to `i32` (`arith.trunci`), and returns it. The process exit
  status is therefore the low 8 bits of the result (`SEM-PROG-1`).

## Upstream lowering and code generation

- **LOW-UP-1 (v0).** After `idr-lower`, the module contains only `func`,
  `arith`, `scf`, `ub` and `llvm` ops. Upstream `convert-scf-to-cf`,
